...
def get_quote_history(estimator_name=None):
    q = _quotes_df()
    if q.empty: return pd.DataFrame(columns=["name", "estimator", "total_price", "status", "created_at"])
...
def get_followup_queue():
    ...
    return df.sort_values("next_followup_date")
...
def save_new_quote(cust_id, job_type, estimator, items):
    ...
    MOCK_FOLLOWUPS.append({
        "quote_id": qid, "name": cname, "phone": phone,
...
server = app.server  # <--- CRITICAL FOR RENDER
...
def generate_pdf(quote_id, customer_name, items, total):
    ...
    # Save to a temporary buffer instead of file system (better for web)
    # For now, simplistic string return for the callback
    # In a real app, you'd stream this. For Render file system:
...
history_tab = dbc.Container([
    ...
    dash_table.DataTable(
        id="history-table",
        columns=[{"name": "Client", "id": "name"}, {"name": "Estimator", "id": "estimator"}, {"name": "Total", "id": "total_price", "type": "numeric", "format": dash_table.FormatTemplate.money(2)}, {"name": "Status", "id": "status"}, {"name": "Created", "id": "created_at"}],
...
def update_kpis(tab, *_):
    ...
    open_count = int((q["status"] == "Open").sum()) if not q.empty else 0
    fup_today = int((fup["next_followup_date"] == date.today().strftime("%Y-%m-%d")).sum()) if not fup.empty else 0
